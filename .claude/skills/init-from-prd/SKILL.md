...
Ask the user for the project slug if not evident from the document (kebab-case, e.g., `ai-accountant`, `pr-pros-ua`). This will be the directory name under `projects/`.

Check whether `projects/{slug}/config.yaml` exists:
...
- If it does not exist: tell the user "No config found — will create a new one from scratch." If no per-project template exists, copy structure from `viral-factory/viral-factory.config.yaml`.
...
**Niche slug** — use the slug provided by the user (confirmed in Pre-flight).
...
## Step 6 — Write or update per-project config

Construct the complete `projects/{slug}/config.yaml` content:

```yaml
# Проєкт: {slug} / {product name}
# Generated by /init-from-prd from: {prd_filename}
# Date: {date}
# Щоб зробити цей проєкт активним: встановіть active_project: {slug} у viral-factory.config.yaml
# Review all TODO comments before running /discover-niche

niche:
  slug: {derived slug — same as project slug or a niche-specific sub-slug}
  keywords:
...
Write the final config to `projects/{slug}/config.yaml`.

---

## Step 7 — Activate the project

Update `viral-factory.config.yaml` in the repo root:
- Set `active_project: {slug}`
- If the file doesn't exist yet, create it with just that line plus a comment

Tell the user: "Project '{slug}' is now active. All pipeline commands will read `projects/{slug}/config.yaml` and write artifacts to `artifacts/viral-factory/{slug}/`."
...
Tell the user:
1. Config written to `projects/{slug}/config.yaml`
2. Active project set to `{slug}` in `viral-factory.config.yaml`
3. Niche slug and number of keywords extracted
4. Number of gaps remaining (with the gap table from Step 8)
